...
type CreateContentAITaskRunBody = {
  site_id?: string | null
...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const tasks = await content.listContentAITaskRuns(
    {
...
export const POST = async (
  req: MedusaRequest<CreateContentAITaskRunBody>,
  res: MedusaResponse
) => {
  if (!isPlatformPluginEnabled("content-core")) {
    localizedError(req, res, 503, "content.disabled")
  ...
  const body = (req.validatedBody || req.body) as CreateContentAITaskRunBody
  const content = resolveContentCoreService(req.scope)
  const task = await content.createAITaskRunSafe({
    siteId: body.site_id,
