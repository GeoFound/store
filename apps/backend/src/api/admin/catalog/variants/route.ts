...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  const queryParams = (req.validatedQuery || req.query) as Record<string, unknown>
  const limit = toBoundedNumber(queryParams.limit, DEFAULT_LIMIT, MAX_LIMIT)
  const offset = toBoundedNumber(queryParams.offset, 0, 10_000)
  const search = toOptionalString(queryParams.q).toLowerCase()
  ...
  const variants = await Promise.all(
    rows.map((row) => toCatalogVariant(req.scope, row, locale))
  )
  const availabilityByVariantId = await listAvailability(req.scope, variants)
