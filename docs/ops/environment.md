...
| `AI_PROVIDER_CONFIGS_JSON` | JSON array of AI provider configs with `code`, `protocol`, `base_url`, `api_key_env`, `default_model`, and `capabilities` fields | Keep backend-only. Do not put plaintext keys here; reference deployment-owned key env names such as `OPENROUTER_API_KEY`, `AI_RELAY_API_KEY`, or `OPENROUTER_COMPAT_KEY`. Use capabilities such as `text.generate`, `speech.tts`, and `speech.stt` instead of hard-coding vendors. |
| `CONTENT_STORAGE_DEFAULT_PROVIDER` | Default content asset provider code from `CONTENT_STORAGE_PROVIDERS_JSON` | Default `local`. Can point to S3, R2, local, or external storage provider records. |
| `CONTENT_STORAGE_PROVIDERS_JSON` | JSON array of content storage providers with `code`, `kind`, `bucket`, `endpoint`, `public_base_url`, `access_key_id_env`, and `secret_access_key_env` fields | Keep backend-only. Supports `s3` and Cloudflare `r2` side by side. Do not put plaintext keys here; reference deployment-owned env names only. |
| `CONTENT_STORAGE_LOCAL_ROOT` | Local content asset root/bucket label used when no storage providers JSON is configured | Default `local-content-assets`; this mode records assets only and is suitable for development. |
| `CONTENT_STORAGE_PUBLIC_BASE_URL` | Optional public base URL for local content asset references | Optional. Use a CDN or static file origin if local assets are served publicly. |
